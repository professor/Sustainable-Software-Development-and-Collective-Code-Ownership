...
\section {Abstract}
\textif{Context:} Conventional wisdom says that team disruptive should be avoided. However, we have observed that Pivotal projects succeed despite high disruption.  

\textif{Objective:} The purpose of this paper is to understand how Pivotal overcomes team disruption in software development.

\textif{Method:} We followed Constructivist Grounded Theory. The primary researcher conducted participant-observation of several software development projects at Pivotal, and interviewed 21 software engineers, interaction designers, and product managers. The researcher iteratively sampled and analyzed the collected data until achieving theoretical saturation.

\textif{Results:} This paper introduces a Theory of Sustainable Software Development. The theory encompasses principles, policies, and practices aiming at removing knowledge silos and improving code quality (including discoverability and readability), hence leading to development sustainability. 

\textif{Limitations:} While the results are highly relevant to the observed projects at Pivotal, the outcomes may not be transferable to other software development organizations with different software development cultures.

\textif{Conclusion:}  The primary benefits to the engineer are the ability to understand the entire system, the ability to work on every feature, and numerous opportunities to share expertise and learn subtle parts of the systems and technologies. The primary benefit to the employer is business agility. The engineering team continues to deliver software week after week, month after month, regardless of changes in the team composition. The business is able to achieve development sustainability even in the face of team disruption.
...
Conventional wisdom says that team disruption should be avoided. Yet, team disruption is a reality in the industry, as exemplified by Project Quattuor where only four of twenty-two developers worked on the project for from beginning to end (see Figure \ref{DeveloperStaffing}.) However, the observed organization keeps a positive attitude towards disruption, transforming a challenge into an opportunity and hence demonstrating remarkable business agility. Team members rolling off the project were replaced as needed. New members rolling onto the project were viewed as an opportunity to improve the current code base by providing a fresh, new perspective. When a new team member did not understand the code base, he or she revealed issues with code discoverability. New team members often questioned the team's assumptions and challenged \quotes{cargo culting.} 
...
\textif(Anything goes:} A few teams allow developers to pick with whom they will work or on which stories to work based on individual preferences. This has the same downsides as the previous strategy. 

\textif(Optimizing for context sharing:} A few teams are experimenting with rotating the last person to work on that track. The goal each day is for the developer leaving the track next to empower the developer joining the track. When the rotation happens the next day, the second developer is asked, \quotes{Was enough context shared with you?} If the answer is no, then the pair continues to work together. This provides a feedback loop on how well the team is transferring knowledge.  
...
Yelling an update to the entire team might be a simple as saying \quotes{The build is broken, we are looking into it}, or this interchange:  \quotes{We just checked in a presenter} followed by \quotes{We just used your presenter. That's great collaboration.}
...
transitioning into
onboarding developers switch from \quotes{I made this} (personal ownership) to \quotes{we made this} (communal ownership) pride in workmanship
...
Shared Schedule & Location generates opportunities for team members to share information, hence supporting the practices aiming at removing knowledge silos, as described in the following sections.

Technical Debt refers to delayed technical work that is incurred when technical shortcuts are taken, usually in pursuit of calendar-driven software schedules  \cite{McConnellTechnicalDebt}. One way to prevent technical debt from accumulating, is to invest in continuous improvement, in the form of refactoring for instance (as described under the Continuous Refactoring practice.) When a team employs continuous refactoring, the team refines a code base towards simple design, intention revealing code, and discoverable code. 
